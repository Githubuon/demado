...
export function MuteButton({
  ...
  return (
    <div
      ...
      onClick={(ev) => {
        ev.stopPropagation();
        launcher.mute(mado, !muted).then(refresh);
      }}
    >
...
export function CameraButton({
  ...
  useEffect(() => {
    perm.capture.granted().then(setGranted);
  }, [perm, setGranted]);
  if (!mado.$existance) return null;
  ...
  return (
    <div
      className="icon"
      title={
        granted ? "スクリーンショットを撮る" : "スクショには許可が必要です"
      }
      onClick={async (ev) => {
        ev.stopPropagation();
    ...
    >
      <i className={granted ? "fa fa-camera" : "fa fa-exclamation-circle"} />
    </div>
  );
}
...
function SubPageButton({
  ...
  return (
    <div
      ...
      onClick={async (ev) => {
        ev.stopPropagation();
        await launcher.launchSubPage(mado, page);
      }}
    >
      {page.iconUrl ? (
        <img
...
export function EmptyShortCard({
  ...
  const an = config.isAnnounceEffective(
    releasenote.announce,
    releasenote.releases[0].version,
  );
  return (
    <div
      className="demado-short-card demado-empty-state"
      onClick={() => window.open(chrome.runtime.getURL("index.html#options"))}
    >
      <div className="level is-mobile">
        <div
          className={
            "level-item " + (an ? "has-text-warning is-shaking" : "has-text-")
          }
        >
...
export function OpenDashboardButton({ launcher }: { launcher: MadoLauncher }) {
  return (
    <div
      className="demado-dashboard-opener-container is-clickable"
      title="ダッシュボードを開く"
      onClick={async () => {
        await launcher.dashboard.open();
        window.close();
      }}
    >
